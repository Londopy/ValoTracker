...
`vt` is an independent, open-source tool created for personal use and educational
...
`vt` is not affiliated with, endorsed by, sponsored by, or in any way officially
connected to Riot Games, Inc. "VALORANT," "Riot Games," and all related names,
marks, and logos are trademarks of Riot Games, Inc.

---

## How vt Works (and Why It Should Be Fine)

`vt` works exclusively by reading data from **VALORANT's own local client API** —
...
- **Read-only** — `vt` never writes to, modifies, or injects into the game
  process, game files, or any Riot service
- **Credential-free** — `vt` reads an authentication token that Riot's own
...
- **Local-only** — all data processing happens on your machine; `vt` makes
...
This places `vt` in the same category as many other community tools (Blitz,
...
Before using `vt`, you are responsible for:
...
3. Keeping `vt` up to date — if Riot changes their API or policy, an
   outdated version of `vt` may behave in ways that were not originally intended
...
and contributors of `vt` from any claim, damage, loss, or expense (including

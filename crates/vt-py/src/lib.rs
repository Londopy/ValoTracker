...
impl VtClient {
    ...
    fn wait_for_match(&self) -> PyResult<()> {
        self.rt
            ...
            .map_err(|e: vt_core::VtError| PyRuntimeError::new_err(e.to_string()))
    }
}

// ── Module registration ───────────────────────────────────────────────────────

/// Python extension module `vt._vt`
#[pymodule]
fn vt(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<PyPlayer>()?;
    m.add_class::<VtClient>()?;
    Ok(())
}

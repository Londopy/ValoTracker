...
/// Full application configuration, stored at `%APPDATA%\vt\config.toml`.
...
impl Config {
    /// Load config from `%APPDATA%\vt\config.toml`.
    ///
    /// Returns default config if the file does not exist yet.
    pub fn load() -> Result<Self, VtError> {
        let path = Self::path()?;
        if !path.exists() {
    ...
    /// Save config to `%APPDATA%\vt\config.toml`, creating the directory if needed.
    ...
    fn path() -> Result<PathBuf, VtError> {
        ...
        Ok(PathBuf::from(appdata).join("vt").join("config.toml"))
    }
}

...
impl Lockfile {
    ...
    pub fn read() -> Result<Self, VtError> {
        let path = Self::path()?;
        let raw = std::fs::read_to_string(&path).map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                VtError::LockfileNotFound
            } else {
                VtError::Io(e)
            }
        })?;
    ...
    pub fn read_from(path: &std::path::Path) -> Result<Self, VtError> {
        let raw = std::fs::read_to_string(path).map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                VtError::LockfileNotFound
            } else {
    ...
    fn parse(raw: &str) -> Result<Self, VtError> {
        let parts: Vec<&str> = raw.trim().splitn(5, ':').collect();
        if parts.len() < 5 {
            return Err(VtError::other(format!(
        ...
        let port = parts[2]
            .parse::<u16>()
            .map_err(|e| VtError::other(format!("Invalid port in lockfile: {e}")))?;
        Ok(Lockfile {
            port,
    ...
    fn path() -> Result<PathBuf, VtError> {
        let local_app_data = std::env::var("LOCALAPPDATA")
            .map_err(|_| VtError::other("LOCALAPPDATA environment variable not set"))?;

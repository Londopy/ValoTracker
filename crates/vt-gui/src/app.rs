//! vt-gui — egui desktop application.
//!
//! Architecture
//! ────────────
...
use eframe::egui;
use vt_core::{
    Config, MatchSnapshot, ResolvedPlayer,
    history::{MatchHistory, PlayerEncounter, SavedMatch},
    tier_to_name, tier_to_short,
    VtError,
};
...
enum Tab { #[default] Match, History }
...
impl GuiApp {
    ...
    fn save_match_action(&mut self, bg: &BgState) {
        ...
        match MatchHistory::open() {
            Ok(db) => match db.save_match(
                &snap.match_id, &snap.map_name, &snap.queue_id,
                &snap.server, &snap.players, &snap.my_puuid, None,
            ) {
    ...
    fn open_history(&mut self) {
        ...
        self.history_sel = if self.history.is_empty() { None } else { Some(0) };
    }
...
fn bg_thread(
    ...
    rt.block_on(async move {
        ...
        let mut engine = loop {
            match vt_core::engine::Engine::init().await {
                Ok(e)  => break e,
                Err(e) => {
        ...
        loop {
            ...
            loop {
                if rf.swap(false, Ordering::Relaxed) { break; }
...
impl eframe::App for GuiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.show_encounter {
            ...
            if close { self.show_encounter = false; }
        }
        ...
        egui::TopBottomPanel::top("topbar")
            ...
            .show(ctx, |ui| {
                draw_topbar(ui, &bg, &mut self.tab, &mut do_refresh, &mut do_save, &mut do_history);
            });
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                match self.tab {
                    Tab::Match   => draw_match_view(
                        ui, &bg, &self.config,
                        &mut open_enc,
                    ),
                    Tab::History => draw_history_view(
                        ui, &mut self.history,
                        &mut self.history_sel,
                        &mut delete_id,
                    ),
                }
            });
        ...
        if do_refresh  { self.trigger_refresh(); }
        if do_save     { self.save_match_action(&bg); }
        if do_history  { self.open_history(); }
        ...
        if let Some(id) = delete_id {
            if let Ok(db) = MatchHistory::open() { let _ = db.delete_match(&id); }
            self.history.retain(|m| m.id != id);
            if self.history_sel.map(|i| i >= self.history.len()).unwrap_or(false) {
...
fn draw_topbar(
    ...
    ui.horizontal(|ui| {
        ...
        if let Some(snap) = &bg.snapshot {
            ...
            ui.separator();
            ui.label(egui::RichText::new(&snap.queue_id)
                .color(egui::Color32::from_rgb(180, 180, 180)));
            if !snap.server.is_empty() {
                ui.separator();
                ui.label(egui::RichText::new(&snap.server)
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            if ui.add_enabled(!bg.loading, egui::Button::new("⟳ Refresh"))
            ...
            if ui.add_enabled(bg.snapshot.is_some(), egui::Button::new("💾 Save"))
...
fn draw_statusbar(
    ...
    ui.horizontal(|ui| {
        if let Some((msg, _)) = status_msg {
            ...
            ui.label(egui::RichText::new("[⟳] Refresh   [💾] Save   [📋] History   [Esc] Close panel")
                .color(colors::DIM)
                .small());
        }
...
fn draw_match_view(
    ...
    // Column pixel widths
    const W: [f32; 12] = [44.0, 100.0, 185.0, 110.0, 38.0, 90.0, 45.0, 45.0, 46.0, 38.0, 45.0, 35.0];
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            egui::Grid::new("match_grid")
                ...
                .show(ui, |ui| {
                    // ── Column headers ────────────────────────────────────────
                    for (i, h) in ["PTY","AGENT","NAME","RANK","RR","PEAK","HS%","WR%","K/D","LVL","ΔRR","MET"]
                        .iter().enumerate()
                    {
                        ui.add_sized(
                            [W[i], 16.0],
                            egui::Label::new(
                                egui::RichText::new(*h).strong().color(colors::HEADER).small(),
                            ),
                        );
                    }
                    ui.end_row();
                    ...
                    for player in players.iter() {
                        ...
                        // ── Party ─────────────────────────────────────────────
                        let p_col = if player.is_enemy_party
                            && config.display.highlight_enemy_parties
                        {
                            colors::PARTY_ENEMY
                        } else {
                            egui::Color32::from_rgb(180, 180, 220)
                        };
                        let p_str = if player.party_size > 1
                            && config.display.show_party_size
                        {
                        ...
                        let name_col = if player.is_ally { colors::ALLY_COLOR } else { colors::ENEMY_COLOR };
                        ...
                        if player.times_seen > 0 {
                            ...
                            let resp = ui.add_sized(
                                [W[2], 20.0],
                                egui::Button::new(
                                    egui::RichText::new(&lbl).color(name_col),
                                )
                                .frame(false),
                        ...
                        let peak_str = if short {
                            ...
                            if n.len() > 8 { n[..8].to_owned() } else { n.to_owned() }
                        };
                        ui.add_sized(
                            [W[5], 20.0],
                            egui::Label::new(
                        ...
                        // ── Level ─────────────────────────────────────────────
                        let lvl_str = if config.display.show_level
                            && !player.hide_account_level
                        {
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            egui::Grid::new("history_grid")
                ...
                .show(ui, |ui| {
                    // Header
                    for (i, h) in ["Date","Map","Queue","W/L","Rank","ΔRR",""].iter().enumerate() {
                        ui.add_sized(
                            [CW[i], 16.0],
                            egui::Label::new(
                                egui::RichText::new(*h).strong().color(colors::HEADER).small(),
                    ...
                    for (i, m) in history.iter().enumerate() {
                        ...
                        if row_click(ui, &date_str,  colors::DIM)                  { *history_sel = Some(i); }
                        if row_click(ui, &m.map,     egui::Color32::from_rgb(215,215,215)) { *history_sel = Some(i); }
                        if row_click(ui, &m.queue,   egui::Color32::from_rgb(180,180,180)) { *history_sel = Some(i); }
...
fn draw_encounter_panel(
    ...
    // Summary
    let summary = vt_core::history::summarize_encounters(encounters);
    let taunt   = summary.worst_game.as_ref()
    ...
    ui.label(egui::RichText::new(format!(
        ...
        summary.wins_against, summary.losses_against,
        summary.avg_kills, summary.avg_deaths,
            ...
            egui::Grid::new("enc_grid")
                ...
                .show(ui, |ui| {
                    for h in ["Date","Map","Agent","K","D","A","HS%","W/L"] {
                        ui.label(egui::RichText::new(h).strong().color(colors::HEADER).small());
                    }
                    ui.end_row();
                    ...
                    for enc in encounters {
                        ...
                        ui.label(egui::RichText::new(enc.kills.to_string())
                            .color(egui::Color32::from_rgb(80, 220, 100)).monospace().small());
                        ui.label(egui::RichText::new(enc.deaths.to_string())
                            .color(egui::Color32::from_rgb(220, 80, 80)).monospace().small());
                        ui.label(egui::RichText::new(enc.assists.to_string())
                            .color(colors::DIM).monospace().small());
                        ui.label(egui::RichText::new(
                            format!("{:.0}%", enc.hs_pct * 100.0))
                            .color(colors::hs_color(enc.hs_pct)).monospace().small());

...
use crate::{error::ValoTrackerError, models::player::ResolvedPlayer};

/// Local SQLite match history database.
///
/// Stored at `%APPDATA%\ValoTracker\history.db`.
...
impl MatchHistory {
    /// Open or create the history database at `%APPDATA%\ValoTracker\history.db`.
    pub fn open() -> Result<Self, ValoTrackerError> {
...
impl MatchHistory {
    ...
    pub fn delete_match(&self, match_id: &str) -> Result<(), ValoTrackerError> {
        self.conn
...
impl MatchHistory {
    ...
    pub fn list_matches(&self, limit: usize) -> Result<Vec<SavedMatch>, ValoTrackerError> {
    ...
    pub fn get_match_players(&self, match_id: &str) -> Result<Vec<SavedPlayer>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
    ...
    pub fn times_played_with(&self, puuid: &str) -> Result<u32, ValoTrackerError> {
    ...
    pub fn matches_with_player(&self, puuid: &str) -> Result<Vec<SavedMatch>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
            "SELECT m.id, m.map, m.queue, m.server, m.saved_at, m.won,
                    m.my_agent, m.my_rank_tier, m.my_rr, m.my_rr_delta,
                    m.my_kills, m.my_deaths, m.my_assists, m.my_score,
    ...
    pub fn check_smurf_flag(
        ...
        threshold_tiers: u8,
        threshold_days: u32,
    ) -> Result<Option<SmurfFlag>, ValoTrackerError> {
    ...
    pub fn my_agent_stats(&self, puuid: &str) -> Result<Vec<AgentStats>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
            "SELECT mp.agent,
    ...
    pub fn my_map_stats(&self, puuid: &str) -> Result<Vec<MapStats>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
            "SELECT m.map,
    ...
    pub fn get_nemesis(&self, puuid: &str) -> Result<Option<Nemesis>, ValoTrackerError> {
    ...
    pub fn get_rivalries(&self, puuid: &str) -> Result<Vec<Nemesis>, ValoTrackerError> {
...
impl MatchHistory {
    ...
    pub fn my_match_history(
        ...
        queue: Option<&str>,
        limit: usize,
    ) -> Result<Vec<MyMatchResult>, ValoTrackerError> {
    ...
    pub fn my_rr_timeline(&self, puuid: &str) -> Result<Vec<(i64, i32)>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
    ...
    pub fn my_party_winrates(&self, puuid: &str) -> Result<Vec<PartyWinRate>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
    ...
    pub fn my_sessions(&self, puuid: &str) -> Result<Vec<Session>, ValoTrackerError> {
    ...
    pub fn my_hourly_stats(&self, puuid: &str) -> Result<Vec<HourlyStats>, ValoTrackerError> {
        let mut stmt = self.conn.prepare(
